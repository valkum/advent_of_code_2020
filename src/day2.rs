...
pub fn part1(input: &[Entry]) -> usize {
    input.iter().filter(|x| { 
        let len = x.password.bytes().filter(|c| *c == x.char.as_bytes()[0]).count();
        len >= x.min && len <= x.max
    }).count()
}
...
pub fn part2(input: &[Entry]) -> usize {
    input.iter().filter(|x| { 
        (x.password.as_bytes()[x.min-1] == x.char.as_bytes()[0]) ^ (x.password.as_bytes()[x.max-1] == x.char.as_bytes()[0])
    }).count()
}
